  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-bg-secondary'>
      <div className='max-w-md w-full bg-bg-card rounded-xl shadow-lg p-8 text-center'>
        {status === 'loading' && (<><Loader2 className='w-16 h-16 text-blue-600 mx-auto mb-4 animate-spin' /><h1 className='text-2xl font-bold mb-2'>Verifying Payment</h1><p className='text-tx-secondary'>Please wait...</p></>)}
        {status === 'success' && (<><CheckCircle className='w-16 h-16 text-green-500 mx-auto mb-4' /><h1 className='text-2xl font-bold mb-2'>Payment Successful!</h1><p className='text-tx-secondary mb-6'>{message}</p><Link href='/owner/dashboard' className='px-6 py-3 bg-blue-600 text-white rounded-lg inline-block'>Go to Dashboard</Link></>)}
        {status === 'error' && (<><XCircle className='w-16 h-16 text-red-500 mx-auto mb-4' /><h1 className='text-2xl font-bold mb-2'>Payment Failed</h1><p className='text-tx-secondary mb-6'>{message}</p><div className='space-x-2'><Link href='/owner/subscription' className='px-6 py-3 bg-blue-600 text-white rounded-lg inline-block'>Try Again</Link><Link href='/owner/dashboard' className='px-6 py-3 bg-bg-secondary rounded-lg inline-block'>Dashboard</Link></div></>)}
      </div>
    </div>
  );
}
