    ...
    <DashboardLayout role='owner'>
      <div className='space-y-8 max-w-6xl mx-auto'>
        <div className='text-center'>
          <h1 className='text-3xl font-bold text-tx-primary'>Subscription Plans</h1>
          <p className='text-tx-secondary mt-2'>Choose the plan that fits your needs</p>
        </div>
        {subscription && (
        ...
        <div className='flex justify-center'>
          <div className='bg-bg-secondary rounded-lg p-1 inline-flex'>
            <button onClick={() => setBillingCycle('monthly')} className={'px-4 py-2 rounded-md text-sm font-medium ' + (billingCycle === 'monthly' ? 'bg-bg-card shadow' : 'text-tx-muted')}>Monthly</button>
            <button onClick={() => setBillingCycle('yearly')} className={'px-4 py-2 rounded-md text-sm font-medium ' + (billingCycle === 'yearly' ? 'bg-bg-card shadow' : 'text-tx-muted')}>Yearly</button>
          </div>
        </div>
        <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-4'>
          {plans.map((plan) => (
            <div key={plan.id} className={'bg-bg-card rounded-xl shadow-sm border-2 p-6 relative ' + (plan.popular ? 'border-blue-500' : 'border-bd')}>
              ...
              <button onClick={() => handleUpgrade(plan.id)} disabled={upgrading === plan.id || subscription?.plan === plan.id} className={'w-full py-2 rounded-lg font-medium ' + (subscription?.plan === plan.id ? 'bg-bg-secondary text-tx-muted' : plan.popular ? 'bg-blue-600 text-white' : 'bg-bg-secondary')}>
