      ...
      <div className="bg-bg-card shadow">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3"><Wrench className="w-8 h-8 text-orange-600" /><h1 className="text-2xl font-bold text-tx-primary">{request?.title || 'Maintenance Request'}</h1></div>
          </div>
        </div>
      </div>
          ...
          <>
            <div className="bg-bg-card rounded-lg shadow-sm border p-6">
              ...
              <div className="space-y-4">
                <div><p className="text-sm text-tx-muted">Description</p><p className="text-tx-primary">{request.description || 'No description'}</p></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-sm text-tx-muted">Category</p><p className="font-medium text-tx-primary capitalize">{request.category || 'N/A'}</p></div>
                  <div><p className="text-sm text-tx-muted">Reported</p><p className="font-medium text-tx-primary">{request.reported_date || request.created_at ? new Date(request.reported_date || request.created_at).toLocaleDateString() : 'N/A'}</p></div>
                </div>
              </div>
            </div>
            <div className="bg-bg-card rounded-lg shadow-sm border p-6">
              ...
              <div className="flex items-center gap-4">
                <select value={status} onChange={e => setStatus(e.target.value)} className="px-4 py-2 border border-bd-strong rounded-lg text-tx-primary bg-bg-card">
