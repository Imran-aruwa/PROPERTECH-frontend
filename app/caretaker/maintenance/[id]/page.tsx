      ...
      <div className="bg-bg-card shadow">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4">
            <Link href="/caretaker/maintenance" className="p-2 hover:bg-bg-hover rounded-lg"><ArrowLeft className="w-5 h-5 text-tx-secondary" /></Link>
            <div className="flex items-center gap-3"><Wrench className="w-8 h-8 text-teal-600" /><h1 className="text-2xl font-bold text-tx-primary">{request?.title || 'Maintenance Request'}</h1></div>
          </div>
        </div>
      </div>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        {!request ? (
          <div className="bg-bg-card rounded-lg shadow-sm border p-8 text-center"><p className="text-tx-muted">Request not found</p></div>
        ) : (
          <>
            <div className="bg-bg-card rounded-lg shadow-sm border p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-tx-primary">Details</h2>
              ...
              <p className="text-tx-primary mb-4">{request.description || 'No description'}</p>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-tx-muted">Category:</span> <span className="text-tx-primary capitalize">{request.category || 'N/A'}</span></div>
                <div><span className="text-tx-muted">Priority:</span> <span className="text-tx-primary capitalize">{request.priority || 'N/A'}</span></div>
              </div>
            </div>
            <div className="bg-bg-card rounded-lg shadow-sm border p-6">
              <h2 className="text-lg font-semibold text-tx-primary mb-4">Update Status</h2>
              <div className="space-y-4">
                <select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-4 py-2 border border-bd-strong rounded-lg text-tx-primary bg-bg-card">
                ...
                <div><label className="block text-sm font-medium text-tx-secondary mb-1">Resolution Notes</label><textarea value={resolution} onChange={e => setResolution(e.target.value)} rows={3} className="w-full px-4 py-2 border border-bd-strong rounded-lg text-tx-primary bg-bg-card" placeholder="What was done..." /></div>
