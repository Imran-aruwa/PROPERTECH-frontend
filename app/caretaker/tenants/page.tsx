  ...
  const columns = [
    {
      ...
      accessor: (row: Tenant) => (
        <div>
          ...
          <p className="text-sm text-tx-muted">Unit {row.unit}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      accessor: (row: Tenant) => (
        <div className="text-sm">
          <p>{row.leaseStart || '-'}</p>
          <p className="text-tx-muted">to {row.leaseEnd || '-'}</p>
        </div>
      ),
    },
    {
      ...
      accessor: (row: Tenant) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          row.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-bg-secondary text-tx-primary'
        }`}>
        ...
        <div key={tenant.id} className="bg-bg-card rounded-lg border p-4 space-y-3">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-tx-muted">Unit {tenant.unit}</p>
            </div>
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
              tenant.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-bg-secondary text-tx-primary'
            }`}>
          ...
          <div className="pt-2 border-t grid grid-cols-2 gap-2 text-sm">
            ...
            <div>
              <p className="text-tx-muted">Lease End</p>
  ...
  return (
    <DashboardLayout role="caretaker">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-tx-primary">Tenants Directory</h1>
          <p className="text-tx-secondary mt-1">View and manage tenant information</p>
        </div>
